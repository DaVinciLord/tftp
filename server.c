...
int main(void) {
	...
	while(1) {
		if(tftp_wait_RRQ(sock, client_addr, buffer, filename, &filename_len) != -1) {
			...
			arg->sock = (SocketUDP *)malloc(sizeof(SocketUDP));
			arg->sock = sock;
			// Lancement du thread
			int errnum = 0;
...
void *run(thread_args_type *arg) {
	...
	if(file != NULL) {
		...
		do {
			...
			tftp_send_DATA_wait_ACK(arg->sock, arg->client_addr, num_block, buffer, length);            
			memset(buffer, 0, sizeof(buffer));
			num_block++;
		} while(length >= TFTP_SIZE); 
		printf("\n-------------------------Envoi terminé----------------------------\n");
	} else {
		tftp_send_error(arg->sock, arg->client_addr, FILE_NOT_FOUND, "Le fichier n'existe pas.");
	}
